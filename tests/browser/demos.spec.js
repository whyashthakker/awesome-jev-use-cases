...
test('live panels complete independently, preserve native distributions, and export the exact edited state',async({page},testInfo)=>{
 ...
 await page.route('**/api/evaluate',async route=>{
  ...
  await route.fulfill({json:{provider:body.provider,mode:'live',latencyMs:isJev?125:300,model:isJev?'jev-test':'gpt-4o-mini',usage:{input_tokens:30,output_tokens:10},cost:{status:'estimated',usd:isJev?0.00000126:0.0000105,currency:'USD'},values:{department:'billing',refund:.9,frustration:1.5},answers:isJev?{department:{type:'choice',choice:'billing',confidence:.9,probabilities:{billing:.9,technical:.1,sales:0}}}:{},decision:{label:'Billing',target:'billing'},note:'Mocked transport for browser test'}});
 });
 await page.goto('/use-cases/01-support-ticket-routing/');await expect(page.locator('#key-status')).toContainText('missing');
 ...
 const stream=await download.createReadStream();let body='';for await(const chunk of stream)body+=chunk;const exported=JSON.parse(body);expect(exported.state).toBe(state);expect(exported.results.openai.model).toBe('gpt-4o-mini');expect(exported.results.openai.cost.usd).toBe(0.0000105);
});
